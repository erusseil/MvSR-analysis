...
def func_fried1(X, A, B, C, D):
    ...
    return (
        np.sin(A * X[:, 0] * X[:, 1]) + B * (X[:, 2] - C) ** 2 + D * X[:, 3] + X[:, 4]
    )
...
def generate_data(func, name, Xs, nXs, params, noises, oversample=False):
    ...
    for idx, param in enumerate(params):
        ...
        if len(np.shape(x.T)) == 1:
            ...
            example = np.array(
                [list(x.T[idx2]) + [y[idx2]] for idx2 in range(len(x.T))]
            )
        ...
        for noise in noises:
            ...
            if type(oversample) == list:
                y_noisy = np.concatenate(
                    [
                        list(y_noisy[: int(len(y) / oversample[idx])])
                        for _k in range(oversample[idx])
                    ]
                )
